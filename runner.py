...
bot.VERSION = "CEO-BOT-V21-FOCUSED"
...
def handle_message(m):
    ...
    if low in {"/start", "/help", "hello", "hi"}:
        ...
        bot.telegram_send_message(
            "📊 Metra CEO Intelligence\nداده‌های بازار و داشبورد مدیریتی",
            chat_id,
            True,
        )
        ...
        bot.telegram_send_message(
            "🔄 بازار در حال به‌روزرسانی است."
            if started
            else "⏳ به‌روزرسانی بازار از قبل در حال انجام است.",
            chat_id,
            True,
        )
    else:
        ...
        if self.path in ("/", "/status", "/health"):
            self._send_json(
                200,
                {
                    ...
                    "version": bot.VERSION,
                    "mode": "focused-dashboard-markets",
                },
            )
        else:
